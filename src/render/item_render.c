...
void item_render_all(SDL_Renderer *renderer, const Player *p, const ItemList *il, const float *zbuf, const Texture *ammo_tex, const Texture *health_tex, const Texture *weapon_kit_tex, const Texture *weapon_kit_ak47_tex, int screen_w, int screen_h) {
    ...
    for (int i = 0; i < il->count; i++) {
        ...
        const Texture *tex = (it->type == ITEM_AMMO) ? ammo_tex : (it->type == ITEM_HEALTH) ? health_tex : (it->type == ITEM_WEAPON_KIT_AK47) ? weapon_kit_ak47_tex : weapon_kit_tex;
